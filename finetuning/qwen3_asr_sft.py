...
def add_tokens_and_resize(model, tokenizer, new_tokens: List[str]) -> int:
    ...
    if num_added > 0:
        ...
        # new tokenizer length. Config objects aren't hashable, so dedupe by
        # identity with a plain list instead of a set.
        new_len = len(tokenizer)
        seen_ids = set()
        cfgs = []
        for cfg in (getattr(model, "config", None), getattr(target, "config", None)):
            if cfg is not None and id(cfg) not in seen_ids:
                seen_ids.add(id(cfg))
                cfgs.append(cfg)
